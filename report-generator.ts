import { FrameAnalysisResult, PersonaAnalysis, Issue } from './claude-api';
import { Persona, getPersonaById } from './personas';
...
export class ReportGenerator {
   ...
   */
  generateComparisonReport(comparison: ComparisonData): string {
    const html = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
  ...
  private generateScoreOverview(data: ReportData): string {
  ...
  private generateFrameAnalyses(data: ReportData): string {
    return `
  ...
  private generateFrameSection(frame: FrameAnalysisResult): string {
    return `
  ...
  private generatePersonaAnalyses(analyses: PersonaAnalysis[]): string {
    return analyses.map(analysis => {
      const persona = getPersonaById(analysis.personaId);
      return `
  ...
  private generateFrameIssues(issues: Issue[]): string {
  ...
  private generateComparisonHeader(comparison: ComparisonData): string {
  ...
  private generateComparisonOverview(comparison: ComparisonData): string {
    return `
  ...
  private generateImprovementSummary(comparison: ComparisonData): string {
    return `
      <div class="summary-section">
  ...
  private generateComparisonDetails(comparison: ComparisonData): string {
    return `
   ...
   */
  exportReport(html: string, filename: string = 'synthetic-testing-report.html'): void {
  ...
  /**
   * Export report as PDF (requires print dialog)
   */
  exportAsPDF(): void {
    window.print();
  }

  private getStatusFromScore(score: number): 'red' | 'yellow' | 'green' {
    if (score >= 80) return 'green';
    if (score >= 60) return 'yellow';
    return 'red';
  }

  private calculateCategoryAverages(frameResults: FrameAnalysisResult[]): Record<string, number> {
  ...
  /**
   * Create comparison data from two report datasets
   */
  createComparison(before: ReportData, after: ReportData): ComparisonData {
  ...
  private getAllIssues(frameResults: FrameAnalysisResult[]): Issue[] {
